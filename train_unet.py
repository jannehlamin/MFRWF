...
class Trainer(object):
    ...
    def log_experimental_Data(self, filename, data):
        ...
        with open(filename, "w") as train_loss:
            train_loss.writelines("% s\n" % d for d in data)
            train_loss.close()
...
def main(lr):
    ...
    parser.add_argument('--dataset', type=str, default='cweeds',
                        choices=['cweeds', 'bweeds', 'rweeds', 'pweeds'],
    ...
    if args.checkname is None:
        args.checkname = 'original_unet-' + str(args.backbone)
    ...
    for epoch in range(trainer.args.start_epoch, trainer.args.epochs):
        t_loss = trainer.training(epoch)
        train.append(t_loss)
        if not trainer.args.no_val and epoch % args.eval_interval == (args.eval_interval - 1):
            v_loss = trainer.validation(epoch)
            val.append(v_loss)
...
def append_best_model_with_lr(file_path, lr):
    print(file_path)
    path = file_path.split("/")
    filename_path = path[:-1]
    result = ""
    i = 0
    for pat in filename_path:
        result += pat + '/'
        i = i + 1
    newfile = result + str(lr) + "_model_best.pth.tar"
    print("Best model for the lr=", lr, newfile)
...
if __name__ == "__main__":
    ...
    for i in range(len(lrs)):
        ...
        os.rename(directory + "cweeds", directory + str(i) + "_unet_carrotweeds_ori_asp")
