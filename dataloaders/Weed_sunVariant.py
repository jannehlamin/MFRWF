...
from dataloaders.utils import decode_segmap, get_weeds
...
class SunVariantWeed(data.Dataset):
    NUM_CLASSES = 3

    def __init__(self, args, base_dir=Path.db_root_dir('svweeds'), base_to='train'):
...
if __name__ == '__main__':
    ...
    voc_train = SunVariantWeed(args, base_to='test')
    ...
    for ii, sample in enumerate(dataloader):
        ...
        for jj in range(sample["image"].size()[0]):
            ...
            tmp = np.uint8(gt[jj])
            segmap = decode_segmap(tmp, dataset='svweeds')  # dataset='weeds'
