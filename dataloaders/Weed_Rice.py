...
class RiceWeed(data.Dataset):
    NUM_CLASSES = 3

    def __init__(self, args, base_dir=Path.db_root_dir('rweeds'), base_to='train'):

        super().__init__()
        self.base_to = base_to
        self.base_root = os.path.join(base_dir, self.base_to, 'images')
...
if __name__ == '__main__':
    ...
    args.base_size = 513
    args.crop_size = 513
    args.data_comp = "weed"  # suppress

    voc_train = RiceWeed(args, base_to='test')
    ...
    for ii, sample in enumerate(dataloader):
        ...
        for jj in range(sample["image"].size()[0]):
            ...
            tmp = np.uint8(gt[jj])
            segmap = decode_segmap(tmp, dataset='rweeds')  # dataset='weeds'
