...
class CarrotWeed(data.Dataset):
    NUM_CLASSES = 3

    def __init__(self, args, base_dir=Path.db_root_dir('cweeds'), base_to='train'):
        ...
        self.masks = [os.path.join(self.base_root.replace("image", "mask"),
                                   m.split('/')[-1].split('_')[0] + str('_annotation.png')) for m in self.images]
        if self.is_edge:
    ...
    def edgeMask(self, lst_image_edge, status="mask"):
        # # =================================== edge algorithms =======================##
...
if __name__ == '__main__':
    ...
    voc_train = CarrotWeed(args, base_to='val')
    ...
    for ii, sample in enumerate(dataloader):
        ...
        for jj in range(sample["image"].size()[0]):
            ...
            # tmp = np.uint8(gt[jj])
            # segmap = decode_segmap(tmp, dataset='cweeds')  # dataset='weeds'
