...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        # self.cls_head = nn.Conv2d(ocr_key_channels, config.DATASET.NUM_CLASSES, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        feats = torch.cat([x[0], x1, x2, x3], 1)  # from HRNET

        feats = self.conv3x3_ocr(feats)
        # feats = self.cls_head(feats)

        # feats = F.interpolate(feats, size=(12, 12), mode='bilinear', align_corners=True)
        # return feats, [x[0], x1, x2, x3]
        return feats, x2
