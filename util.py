...
class DenoiseSeqConv3x3(nn.Module):
    def __init__(self, in_ch, out_ch, type1='conv1x1-conv3x3', type2='conv1x1-laplacian', depth_mul=-1):
        super(DenoiseSeqConv3x3, self).__init__()
        self.conv = ECB(in_ch, out_ch, 2, act_type='prelu', with_idt=False)
        self.conv3 = nn.Conv2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1)
        self.target = nn.Softmax(dim=1)
